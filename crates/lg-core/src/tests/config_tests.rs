...
fn default_config_has_expected_profile_name() {
    let cfg = Config::default();
    assert_eq!(cfg.profile_name, "lg-ultragear-dynamic-cmx.icm");
}

#[test]
fn default_config_has_expected_icc_gamma() {
    let cfg = Config::default();
    assert_eq!(cfg.icc_gamma, 2.05);
}

#[test]
fn default_config_has_expected_icc_preset() {
    let cfg = Config::default();
    assert_eq!(cfg.icc_active_preset, "gamma22");
}

#[test]
fn default_config_specialized_profiles_enabled() {
    let cfg = Config::default();
    assert!(cfg.icc_generate_specialized_profiles);
}

#[test]
fn default_config_has_expected_luminance() {
    let cfg = Config::default();
    assert_eq!(cfg.icc_luminance_cd_m2, 120.0);
}

#[test]
fn default_config_has_balanced_tuning_preset() {
    let cfg = Config::default();
    assert_eq!(cfg.icc_tuning_preset, "anti_dim_balanced");
    assert!(cfg.icc_tuning_overlay_manual);
}
...
fn serialize_roundtrip() {
    let original = Config {
        ...
        monitor_match_regex: true,
        profile_name: "test.icm".to_string(),
        icc_gamma: 2.05,
        icc_active_preset: "gamma24".to_string(),
        icc_generate_specialized_profiles: true,
        icc_luminance_cd_m2: 160.0,
        icc_tuning_preset: "anti_dim_aggressive".to_string(),
        icc_tuning_overlay_manual: false,
        icc_black_lift: 0.08,
        icc_midtone_boost: 0.12,
        icc_white_compression: 0.25,
        icc_gamma_r: 1.02,
        icc_gamma_g: 1.00,
        icc_gamma_b: 0.98,
        icc_vcgt_enabled: true,
        icc_vcgt_strength: 0.65,
        icc_target_black_cd_m2: 0.3,
        ...
        icc_ciis_signature: "scoe".to_string(),
        icc_cicp_enabled: true,
        icc_cicp_primaries: 1,
        icc_cicp_transfer: 13,
        icc_cicp_matrix: 0,
        icc_cicp_full_range: true,
        icc_metadata_enabled: true,
        ...
        icc_include_spectral_scaffold: true,
    ...
    assert_eq!(parsed.monitor_match_regex, original.monitor_match_regex);
    assert_eq!(parsed.monitor_match_regex, original.monitor_match_regex);
    assert_eq!(parsed.profile_name, original.profile_name);
    assert_eq!(parsed.icc_gamma, original.icc_gamma);
    assert_eq!(parsed.icc_active_preset, original.icc_active_preset);
    assert_eq!(
        parsed.icc_generate_specialized_profiles,
        original.icc_generate_specialized_profiles
    );
    assert_eq!(parsed.icc_luminance_cd_m2, original.icc_luminance_cd_m2);
    assert_eq!(parsed.icc_tuning_preset, original.icc_tuning_preset);
    assert_eq!(
        parsed.icc_tuning_overlay_manual,
        original.icc_tuning_overlay_manual
    );
    assert_eq!(parsed.icc_black_lift, original.icc_black_lift);
    assert_eq!(parsed.icc_midtone_boost, original.icc_midtone_boost);
    assert_eq!(parsed.icc_white_compression, original.icc_white_compression);
    assert_eq!(parsed.icc_gamma_r, original.icc_gamma_r);
    assert_eq!(parsed.icc_gamma_g, original.icc_gamma_g);
    assert_eq!(parsed.icc_gamma_b, original.icc_gamma_b);
    assert_eq!(parsed.icc_vcgt_enabled, original.icc_vcgt_enabled);
    assert_eq!(parsed.icc_vcgt_strength, original.icc_vcgt_strength);
    assert_eq!(
        parsed.icc_target_black_cd_m2,
        original.icc_target_black_cd_m2
    );
    assert_eq!(
        parsed.icc_include_calibration_datetime,
        original.icc_include_calibration_datetime
    );
    assert_eq!(
        parsed.icc_include_chromatic_adaptation,
        original.icc_include_chromatic_adaptation
    );
    assert_eq!(
        parsed.icc_include_chromaticity,
        original.icc_include_chromaticity
    );
    assert_eq!(
        parsed.icc_include_measurement,
        original.icc_include_measurement
    );
    assert_eq!(
        parsed.icc_include_viewing_conditions,
        original.icc_include_viewing_conditions
    );
    assert_eq!(
        parsed.icc_include_spectral_scaffold,
        original.icc_include_spectral_scaffold
    );
    assert_eq!(
        parsed.icc_per_monitor_profiles,
        original.icc_per_monitor_profiles
    );
    assert_eq!(
        parsed.icc_include_media_black_point,
        original.icc_include_media_black_point
    );
    assert_eq!(
        parsed.icc_include_device_descriptions,
        original.icc_include_device_descriptions
    );
    assert_eq!(
        parsed.icc_include_characterization_target,
        original.icc_include_characterization_target
    );
    assert_eq!(
        parsed.icc_include_viewing_cond_desc,
        original.icc_include_viewing_cond_desc
    );
    assert_eq!(
        parsed.icc_technology_signature,
        original.icc_technology_signature
    );
    assert_eq!(parsed.icc_ciis_signature, original.icc_ciis_signature);
    assert_eq!(parsed.icc_cicp_enabled, original.icc_cicp_enabled);
    assert_eq!(parsed.icc_cicp_primaries, original.icc_cicp_primaries);
    assert_eq!(parsed.icc_cicp_transfer, original.icc_cicp_transfer);
    assert_eq!(parsed.icc_cicp_matrix, original.icc_cicp_matrix);
    assert_eq!(parsed.icc_cicp_full_range, original.icc_cicp_full_range);
    assert_eq!(parsed.icc_metadata_enabled, original.icc_metadata_enabled);
    assert_eq!(
        parsed.icc_include_calibration_datetime,
        original.icc_include_calibration_datetime
    );
    ...
    assert_eq!(parsed.icc_cicp_full_range, original.icc_cicp_full_range);
    assert_eq!(parsed.icc_metadata_enabled, original.icc_metadata_enabled);
    assert_eq!(parsed.icc_hdr_preset, original.icc_hdr_preset);
    assert_eq!(parsed.icc_sdr_preset, original.icc_sdr_preset);
    assert_eq!(
        parsed.icc_schedule_day_preset,
        original.icc_schedule_day_preset
    );
    assert_eq!(
...
fn to_toml_commented_contains_all_field_values() {
    ...
    assert!(
        output.contains("monitor_match_regex = false"),
        "should contain monitor_match_regex"
    );
    assert!(
        output.contains("lg-ultragear-dynamic-cmx.icm"),
        "should contain profile_name"
    );
    assert!(
        output.contains("icc_gamma = 2.05"),
        "should contain icc_gamma"
    );
    assert!(
        output.contains("icc_active_preset = \"gamma22\""),
        "should contain icc_active_preset"
    );
    assert!(
        output.contains("icc_generate_specialized_profiles = true"),
        "should contain icc_generate_specialized_profiles"
    );
    assert!(
        output.contains("icc_luminance_cd_m2 = 120"),
        "should contain icc_luminance_cd_m2"
    );
    assert!(
        output.contains("icc_tuning_preset = \"anti_dim_balanced\""),
        "should contain icc_tuning_preset"
    );
    assert!(
        output.contains("icc_tuning_overlay_manual = true"),
        "should contain icc_tuning_overlay_manual"
    );
    assert!(
        output.contains("icc_black_lift = 0"),
        "should contain icc_black_lift"
    );
    assert!(
        output.contains("icc_midtone_boost = 0"),
        "should contain icc_midtone_boost"
    );
    assert!(
        output.contains("icc_white_compression = 0"),
        "should contain icc_white_compression"
    );
    assert!(
        output.contains("icc_vcgt_enabled = false"),
        "should contain icc_vcgt_enabled"
    );
    assert!(
        output.contains("icc_target_black_cd_m2 = 0.2"),
        "should contain icc_target_black_cd_m2"
    );
    assert!(
        output.contains("icc_include_media_black_point = true"),
        "should contain icc_include_media_black_point"
    );
    assert!(
        output.contains("icc_include_device_descriptions = true"),
        "should contain icc_include_device_descriptions"
    );
    assert!(
        output.contains("icc_technology_signature = \"vidm\""),
        "should contain icc_technology_signature"
    );
    assert!(
        output.contains("icc_cicp_enabled = false"),
        "should contain icc_cicp_enabled"
    );
    assert!(
        output.contains("icc_include_calibration_datetime = true"),
        "should contain icc_include_calibration_datetime"
    );
    assert!(
        output.contains("icc_include_chromatic_adaptation = true"),
        "should contain icc_include_chromatic_adaptation"
    );
    assert!(
        output.contains("icc_include_chromaticity = true"),
        "should contain icc_include_chromaticity"
    );
    assert!(
        output.contains("icc_include_measurement = true"),
        "should contain icc_include_measurement"
    );
    assert!(
        output.contains("icc_include_viewing_conditions = true"),
        "should contain icc_include_viewing_conditions"
    );
    assert!(
        output.contains("icc_include_spectral_scaffold = false"),
        "should contain icc_include_spectral_scaffold"
    );
    assert!(
        output.contains("icc_per_monitor_profiles = true"),
        "should contain icc_per_monitor_profiles"
    );
    assert!(
...
fn to_toml_commented_roundtrip_preserves_values() {
    let original = Config {
        ...
        icc_generate_specialized_profiles: false,
        icc_luminance_cd_m2: 135.0,
        icc_tuning_preset: "anti_dim_soft".to_string(),
        icc_tuning_overlay_manual: true,
        icc_black_lift: 0.02,
        icc_midtone_boost: 0.05,
        icc_white_compression: 0.10,
        icc_gamma_r: 1.0,
        icc_gamma_g: 1.0,
        icc_gamma_b: 1.0,
        ...
        icc_include_media_black_point: true,
        icc_include_device_descriptions: false,
        icc_include_characterization_target: true,
        icc_include_viewing_cond_desc: false,
        icc_technology_signature: "vidm".to_string(),
