...
#[test]
fn icc_help_lists_optimize_subcommand() {
    let (stdout, stderr, success) = run_binary(&["icc", "--help"]);
    assert!(success, "icc --help should succeed. stderr: {}", stderr);
    assert!(
        stdout.to_lowercase().contains("optimize"),
        "icc --help should list optimize: {}",
        stdout
    );
}

#[test]
fn icc_optimize_help_shows_expected_options() {
    let (stdout, stderr, success) = run_binary(&["icc", "optimize", "--help"]);
    assert!(
        success,
        "icc optimize --help should succeed. stderr: {}",
        stderr
    );
    let lower = stdout.to_lowercase();
    assert!(
        lower.contains("tuning-preset"),
        "icc optimize help missing --tuning-preset: {}",
        stdout
    );
    assert!(
        lower.contains("--apply"),
        "icc optimize help missing --apply: {}",
        stdout
    );
    assert!(
        lower.contains("save-config"),
        "icc optimize help missing --save-config: {}",
        stdout
    );
    assert!(
        lower.contains("--output"),
        "icc optimize help missing --output: {}",
        stdout
    );
}

#[test]
fn ddc_help_lists_map_subcommand() {
    let (stdout, stderr, success) = run_binary(&["ddc", "--help"]);
    assert!(success, "ddc --help should succeed. stderr: {}", stderr);
    assert!(
        stdout.to_lowercase().contains("map"),
        "ddc --help should list map subcommand: {}",
        stdout
    );
}
...
fn automation_help_lists_subcommands() {
    ...
    assert!(lower.contains("applynow") || lower.contains("apply-now"));
}

#[test]
fn tray_help_lists_run() {
    let (stdout, stderr, success) = run_binary(&["tray", "--help"]);
    assert!(success, "tray --help should succeed. stderr: {}", stderr);
    assert!(stdout.to_lowercase().contains("run"));
}

#[test]
fn bundle_help_lists_export_import() {
    let (stdout, stderr, success) = run_binary(&["bundle", "--help"]);
    assert!(success, "bundle --help should succeed. stderr: {}", stderr);
    let lower = stdout.to_lowercase();
...
#[test]
fn automation_path_command_outputs_automation_toml_path() {
    let (stdout, stderr, success) = run_binary(&["automation", "path"]);
    assert!(
        success,
        "automation path should succeed. stderr: {}",
        stderr
    );
    assert!(
        stdout.to_lowercase().contains("automation.toml"),
        "automation path should contain automation.toml: {}",
        stdout
    );
}

#[test]
fn automation_show_command_outputs_sections() {
    let (stdout, stderr, success) = run_binary(&["automation", "show"]);
    assert!(
        success,
        "automation show should succeed. stderr: {}",
        stderr
    );
    let lower = stdout.to_lowercase();
    assert!(lower.contains("ambient"));
    assert!(lower.contains("app_rules"));
    assert!(lower.contains("ddc_safety"));
}

#[test]
fn automation_reset_dry_run_succeeds() {
    let (stdout, stderr, success) = run_binary(&["--dry-run", "automation", "reset"]);
    assert!(
        success,
        "dry-run automation reset should succeed. stderr: {}",
        stderr
    );
...
#[test]
fn automation_apply_now_dry_run_succeeds() {
    let (stdout, stderr, success) = run_binary(&["--dry-run", "automation", "apply-now"]);
    assert!(
        success,
        "dry-run automation apply-now should succeed. stderr: {}",
        stderr
    );
    let lower = stdout.to_lowercase();
    assert!(lower.contains("dry run") || lower.contains("would reapply profile"));
}

#[test]
fn tray_run_dry_run_succeeds() {
    let (stdout, stderr, success) = run_binary(&["--dry-run", "tray", "run"]);
    assert!(
        success,
        "dry-run tray run should succeed. stderr: {}",
        stderr
    );
    assert!(stdout.to_lowercase().contains("dry run"));
}
...
#[test]
fn icc_optimize_dry_run_does_not_apply_for_real() {
    let (stdout, stderr, success) = run_binary(&[
        "--dry-run",
        "icc",
        "optimize",
    ...
    assert!(
        success,
        "dry-run icc optimize should succeed. stderr: {}",
        stderr
    );
    assert!(
        stdout.contains("[DRY RUN]"),
        "dry-run icc optimize should print dry-run markers: {}",
        stdout
    );
    assert!(
        !stdout.contains("[OK] Applied optimized ICC"),
        "dry-run icc optimize must not report real apply: {}",
        stdout
    );
}
...
fn config_show_displays_all_fields() {
    ...
    assert!(stdout.contains("icc_gamma"));
    assert!(stdout.contains("icc_tuning_preset"));
    assert!(stdout.contains("icc_tuning_overlay_manual"));
...
fn icm_embedded_profile_has_nonzero_size() {
    assert!(
        ...
        "Generated ICM should contain data"
    );
}
...
fn icm_embedded_profile_is_reasonable_size() {
    ...
    assert!(generated_icm_size() > 100, "Generated ICM too small");
    assert!(
        generated_icm_size() < 500_000,
        "Generated ICM suspiciously large"
    );
}
...
fn icm_extract_to_temp_directory() {
    ...
    assert_eq!(
        ...
        "file size should match generated profile size"
    );
...
fn icm_extracted_profile_content_matches_expected_size() {
    ...
    assert_eq!(contents.len(), generated_icm_size());

    // ICC profiles have "acsp" at offset 36 (ICC specification)
    if generated_icm_size() > 40 {
...
fn config_profile_path_matches_expected_format() {
    ...
    assert!(path_str.ends_with("lg-ultragear-dynamic-cmx.icm"));
}
...
fn binary_help_lists_all_subcommands() {
    ...
    assert!(lower.contains("icc"), "help should list icc");
}
...
fn embedded_icm_size_is_reasonable() {
    // ICC profiles are typically at least a few hundred bytes.
    assert!(generated_icm_size() > 100, "ICM should be > 100 bytes");
    assert!(generated_icm_size() < 10_000_000, "ICM should be < 10MB");
}

#[test]
fn embedded_icm_size_is_consistent_with_installed_file() {
    // Install to temp, verify file size matches generated ICC size
    ...
    assert_eq!(
        file_size,
        generated_icm_size() as u64,
        "installed file size should equal generated ICM size"
    );

    let _ = std::fs::remove_dir_all(&dir);
}

// ── Public API surface from integration ──────────────────────────

#[test]
fn embedded_icm_size_constant_is_public() {
    let size = generated_icm_size();
    assert!(size > 0, "generated ICM size should be positive");
}
