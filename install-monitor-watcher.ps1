<#
.SYNOPSIS
  Creates a lightweight persistent monitor for display reconnection events that auto-reapplies the color profile.

.DESCRIPTION
  Installs a scheduled task that triggers on display device connection events and automatically
  reapplies the LG UltraGear color profile. This ensures the fix persists across:
...
.SOLUTION APPROACH
  Uses Windows Event Log monitoring (System log, PlugPlayManager source) which is extremely
  lightweight - only triggers when actual display device events occur.

.USAGE
  PS> .\install-monitor-watcher.ps1
  PS> .\install-monitor-watcher.ps1 -Uninstall

.NOTES
  Requires Administrator privileges to create scheduled task.
#>
...
param(
    ...
    [string]$InstallerPath = "$PSScriptRoot\install-lg-ultragear-no-dimming.ps1",
...
# Log to event log for debugging (optional)
# `$eventLog = 'Application'
# `$eventSource = 'LG-UltraGear-Monitor'
...
# Write-EventLog -LogName `$eventLog -Source `$eventSource -EventId 1000 -EntryType Information -Message 'Color profile reapplied'
"@
...
# Trigger 1: On any display device arrival (Event ID 20001, 20003 from PlugPlayManager)
# This covers monitor connect/reconnect events
...
try {
    ...
    Write-Host ""
    Write-Host "[TIP ] To uninstall: .\install-monitor-watcher.ps1 -Uninstall" -ForegroundColor Gray
